...
def forward_step(data_iterator, model: HybridModel):
    ...
    if cu_seqlens is not None:
        ...
        # Keep real metadata compact until the graph boundary. Capture can clamp
        # its effective capacity for CP alignment, and each replay path pads to
        # that exact capacity (or falls back eagerly when the batch does not fit).
